...
Ссылки Hysteria2 с `obfs=gecko` временно обслуживаются встроенным официальным
...
Исходная `hy2://`-ссылка передаётся Hysteria без повторной конвертации, поэтому
параметры Gecko и `pinSHA256` не теряются.
