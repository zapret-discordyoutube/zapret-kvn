...
Исходная ссылка передаётся Hysteria буквально, без повторной конвертации, поэтому
`pinSHA256`, ECH, port hopping, Gecko и другие параметры URI не удаляются
приложением. Старые ссылки `hysteria://` (Hysteria v1) и native outbound JSON
остаются в нативном пути sing-box.
