...
Экспорт на странице логов содержит состояние, исходные ошибки и
`runtime_redacted.json` — снимки последних JSON, записанных менеджерами ядер.
В них сохраняются DNS, маршруты, адреса и detour; ключи, пароли и URI скрываются.
Время снимка, состояние процесса и контекст сессии помогают отличить старую
попытку от текущей. Снимок не пересобирается из шаблона и сам по себе не означает
успешного подключения; если ядро ещё не записывало конфигурацию, снимка нет.
...
`build_core_bundle.ps1` и `build.py` используют общий каталог
`.cache/core-downloads`: архивы ядер и правил сохраняются между сборками, включая
dev/stable. Перед повторным использованием проверяется SHA-256 из lock-файла;
повреждённый архив скачивается заново. Незавершённая загрузка не становится
готовой записью кэша. Проверка новых версий на GitHub остаётся независимой от
кэша; он не входит в дистрибутив и не удаляется командой `python build.py --clean`.
