...
class TestAwg3ConfImport(unittest.TestCase):
    ...
    def test_awg31_flags_are_preserved_including_false(self) -> None:
        for value, expected in (("on", True), ("true", True), ("off", False), ("0", False)):
            conf = FIXTURE_AWG3_CONF.replace("[Peer]", f"RandomTrailers = {value}\nDisableCookies = {value}\n[Peer]")
            nodes, errors = parse_links_text(conf)
            self.assertEqual(errors, [])
            self.assertEqual(nodes[0].outbound["amnezia"]["random_trailers"], expected)
            self.assertEqual(nodes[0].outbound["amnezia"]["disable_cookies"], expected)
        nodes, errors = parse_links_text(FIXTURE_AWG3_CONF.replace("[Peer]", "RandomTrailers = maybe\n[Peer]"))
