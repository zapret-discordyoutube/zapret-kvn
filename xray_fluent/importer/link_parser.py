    ...
    if native_type and not protocol:
        if native_type == "wireguard":
            ...
            if isinstance(amnezia, dict):
                ...
                if hpk is not None:
                    try:
                        _wg_key(str(hpk), "HeaderProtectionKey")
...
# Official AWG 3.1 flags retain explicit false as well as true.
_AMNEZIA_AWG31_BOOLEAN_KEYS = {"randomtrailers": "random_trailers", "disablecookies": "disable_cookies"}
...
_AMNEZIA_JSON_RANGE_KEYS = (*_AMNEZIA_INT_OR_STR_KEYS, *_AMNEZIA_AWG3_RANGE_KEYS.values())
_AWG_UINT32_MAX = 0xFFFFFFFF
_AWG_RANGE_RE = re.compile(r"^([0-9]+)-([0-9]+)$")
...
def _wg_key(value: str, field: str) -> str:
    """Canonical base64 for a 32-byte key; errors never echo key material."""
    text = str(value or "").strip()
    try:
        raw = base64.b64decode(text + "=" * (-len(text) % 4), validate=True)
    except (binascii.Error, ValueError) as exc:
        raise LinkParseError(
            f"некорректный {field}: не base64"
        ) from exc
    if len(raw) != 32:
        raise LinkParseError(
            f"некорректный {field}: {len(raw)} байт вместо 32"
        )
    return base64.b64encode(raw).decode("ascii")


def _wg_uint32_range(value: Any, field: str) -> int | str:
    """AWG 3.0 диапазон: одно число или "A-B" в пределах uint32, A <= B."""
    if isinstance(value, dict):
        if set(value) != {"from", "to"} or any(type(item) is not int or not 0 <= item <= _AWG_UINT32_MAX for item in value.values()):
            raise LinkParseError(f"некорректный диапазон в поле {field}: ожидаются from/to UInt32")
        value = f'{value["from"]}-{value["to"]}'
    if isinstance(value, bool):
        raise LinkParseError(f"поле {field} должно быть числом или диапазоном")
...
def _parse_amnezia_params(interface: dict[str, str]) -> dict[str, Any]:
    ...
    for key in _AMNEZIA_INT_KEYS:
        if key in interface:
            ...
            limit = 65535 if key.startswith("s") else _AWG_UINT32_MAX
            if not 0 <= amnezia[key] <= limit:
                raise LinkParseError(f"параметр {key} вне диапазона 0..{limit}")
    for key in _AMNEZIA_INT_OR_STR_KEYS:
        if key in interface:
            amnezia[key] = _wg_uint32_range(interface[key], key)
    for key in _AMNEZIA_STR_KEYS:
        if key in interface:
    ...
    for key, json_key in _AMNEZIA_AWG31_BOOLEAN_KEYS.items():
        if key not in interface:
            continue
        value = interface[key].strip().lower()
        if value not in {"true", "false", "t", "f", "1", "0", "on", "off"}:
            raise LinkParseError(f"параметр {key} должен быть true или false")
        amnezia[json_key] = value in {"true", "t", "1", "on"}
    if "headerprotectionkey" in interface:
        amnezia["header_protection_key"] = _wg_key(
...
def _parse_wireguard_conf(text: str) -> Node:
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            ...
            if section == "interface":
                if seen_interface:
                    raise LinkParseError("секция [Interface] указана несколько раз")
                seen_interface = True
                current = interface
            elif section == "peer":
                if not seen_interface:
                    raise LinkParseError("секция [Peer] указана раньше [Interface]")
                current = {}
        ...
        if key in current:
            if key in {"address", "dns", "allowedips"}:
                ...
                continue
            raise LinkParseError(f"параметр {key} указан несколько раз")
        current[key] = value.strip()

    known_interface = {"privatekey", "address", "dns", "mtu", "listenport"} | set(_AMNEZIA_INT_KEYS) | set(_AMNEZIA_INT_OR_STR_KEYS) | set(_AMNEZIA_STR_KEYS) | set(_AMNEZIA_REMOVED_KEYS) | set(_AMNEZIA_AWG3_RANGE_KEYS) | set(_AMNEZIA_AWG31_BOOLEAN_KEYS) | {"headerprotectionkey"}
    unknown = set(interface) - known_interface
    if unknown:
        raise LinkParseError(f"неподдерживаемые параметры [Interface]: {', '.join(sorted(unknown))}")
    for raw_peer in raw_peers:
        unknown = set(raw_peer) - {"endpoint", "publickey", "presharedkey", "allowedips", "persistentkeepalive"}
        if unknown:
            raise LinkParseError(f"неподдерживаемые параметры [Peer]: {', '.join(sorted(unknown))}")
    ...
    outbound["private_key"] = _wg_key(private_key, "PrivateKey")
    if "mtu" in interface:
    ...
    for raw_peer in raw_peers:
        ...
        peer["public_key"] = _wg_key(public_key, "PublicKey")
        pre_shared_key = raw_peer.get("presharedkey", "")
        if pre_shared_key:
            peer["pre_shared_key"] = _wg_key(pre_shared_key, "PresharedKey")
        allowed_ips = [item.strip() for item in raw_peer.get("allowedips", "").split(",") if item.strip()]
        if allowed_ips:
            peer["allowed_ips"] = allowed_ips
        if "persistentkeepalive" in raw_peer:
            value = raw_peer["persistentkeepalive"]
            peer["persistent_keepalive_interval"] = 0 if value.lower() == "off" else _wg_uint32_range(value, "PersistentKeepalive")
        peers.append(peer)
